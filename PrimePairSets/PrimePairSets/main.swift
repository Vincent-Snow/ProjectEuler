...
iLoop: for i in sieve.permutations(ofCount: 2)  where i[0] < i[1]  {
    //jLoop: for j in i.permutations(ofCount: 2) {
        let s1 = String(i[0])
        let s2 = String(i[1])
        let int = Int(s1+s2)!
        if sieveDict[int] != true {
            continue iLoop
        }
    //}
    if s1 == "3" {
    ...
    if primeDict[i[0]]?.count >= 0 {
        primeDict[i[0]]?.append(contentsOf: i)
    } else {
